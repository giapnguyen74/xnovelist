...
export const rephrase: Action<CaptureInput> = {
  id: 'rephrase',
  label: 'Rephrase',
  description: 'Rewrite the selection to improve style and flow while preserving events.',
  level: 2,
  scope: 'selection',
  allow: ['replace_range'],
  async propose(input, ctx): Promise<ProposalResult> {
    ...
    if (!selectionText.trim()) {
      throw new Error('Please select some text to rephrase first.');
    }
