...
export const polishDialogue: Action<CaptureInput> = {
  id: 'polish_dialogue',
  label: 'Polish Dialogue',
  description: 'Sharpen conversation flow and speaker voices.',
  level: 2,
  scope: 'selection',
  allow: ['replace_range'],
  async propose(input, ctx): Promise<ProposalResult> {
    ...
    if (!selectionText.trim()) {
      throw new Error('Please select some text containing dialogue to polish first.');
    }
    ...
    const maxWords = Math.max(5, Math.round(inputWords * 1.15));
